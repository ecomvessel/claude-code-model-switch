<!--
  PRESET: fable-5 — for plans WITH Fable 5 access (e.g. Max).
  Roles: Haiku 4.5 (cheap-reader) / Sonnet 5 (builder) / Opus 4.8 (seat) / Fable 5 (top-tier).
...
2. **Escalate UP to Fable 5, OR any PRODUCTION ACTION = ALWAYS ask first.** "Production action"
...
**Escalate to Fable 5 ONLY for:** (1) architecture/planning a new system, (2) hard or production
...
Fable 5 costs the most — reserve it for design, danger, or money (~20% of work), never routine coding.

At session start, pick the lane from what the task actually is. If the session is on Fable 5 but the
task is UI / CRUD / go-live mechanics, say "this doesn't need the top model, switch down."
...
2. For anything gated by "escalate up, always ask first": if Fable 5 is unavailable, STOP and tell
   the human — do not auto-substitute a weaker model for a security/migration decision. Ask whether
   to proceed on Opus 4.8 or wait.
...
If you lose Fable 5 access permanently (plan change, retirement): switch to the `opus-4-8` preset —
same flow, top tier capped at Opus 4.8.
